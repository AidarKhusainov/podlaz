...
Apply/verify/rollback must be explicit. Normal in-process rollback must remove only what the active transaction recorded as applied. If the daemon stops inside the apply crash window before an applied category is persisted, recovery may reconstruct only that missing category from the durable structured `desired_plan`. Every reconstructed target must still pass the same fixed ownership, interface, route-table, policy-priority, DNS-backend, and nftables guards. Ambiguous host state must be skipped, not guessed.

For `systemd-resolved`, apply first performs a scoped `resolvectl revert podlaz0` and then writes the planned DNS servers, `~.` route-only domain, and DNS default-route setting. An already-missing link is idempotent. Verification checks the target link, planned DNS servers, `~.`, `+DefaultRoute`, and absence of a foreign active `~.` owner. `Current Scopes` is derived runtime state and must not be used as proof that the per-link configuration was or was not applied. Transient propagation is polled for a bounded interval of roughly two seconds.
...
- A stale `systemd-resolved` record that cannot be removed while `podlaz0` is absent must not trigger a global resolver restart. Connect may defer that one scoped repair until Xray has recreated `podlaz0`, then run `resolvectl revert podlaz0` immediately before writing podlaz DNS state.
...
- Unexpected cleanup errors, foreign ownership, invalid transaction files, and incomplete transaction recovery remain blockers.
