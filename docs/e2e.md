...
The issue #247 installed-package acceptance runs after the base package-convergence and resolver acceptance scenarios on the same Ubuntu 24.04 branch package. It is a required disposable-host gate for lifecycle-aware base diagnostics and the product-owned `logs --since` grammar.

The scenario must prove all of the following:

1. With the daemon running and no active connection or podlaz-owned network resources, `podlaz doctor` reports `[OK] stale-resources: no podlaz-owned resources found` and a clean inactive startup recovery scan.
2. After a normal packaged `connect --mode tun`, status is active with a committed transaction and no stale state. Base `podlaz doctor` must accept `podlaz0` only after its current name/ifindex/TUN kind match the persisted expected identity, and must accept `inet podlaz` only after the current `nft -y list table` output passes the same complete canonical verifier used by the networking executor: chain cardinality/name/type/hook/numeric priority/policy and ordered rules must all match. A family/table tuple by itself is never current ownership proof.
3. Before disconnect, the harness adds one uniquely named empty regular chain to the active `inet podlaz` table. The chain has no hook and therefore does not alter packet traversal, but it changes exact table cardinality. Base doctor must emit an actionable `stale-resources` warning for exact-composition mismatch. The harness removes that chain only after a private inspection proves its exact expected empty content; a changed or ambiguous chain fails cleanup closed. After removal, status and doctor must both return to the same clean active committed state.
4. Normal disconnect then returns to the clean inactive diagnostic boundary without restarting `podlazd` or `systemd-resolved`.
5. While inactive, the harness may create one isolated foreign-looking `podlaz0` TUN only after proving the name is absent. Doctor must keep that resource actionable as a stale warning. Cleanup may remove it only when the current ifindex still equals the ifindex recorded immediately after creation and `ip tuntap` still identifies the object as a TUN; identity mismatch fails closed instead of deleting an unknown replacement.
6. The installed package must separately complete `podlaz logs --daemon --since 36h` and `podlaz logs --core --since 36h` through the Ubuntu 24.04 `journalctl` backend with the expected stable heading and without backend timestamp-parse failure. Raw journal output stays private.
7. The harness must prove the requested window is actually enforced rather than merely accepted syntactically. It creates a known recent daemon journal marker, synchronizes journald, waits until that marker is deliberately older than a `1s` request, then runs `podlaz logs --daemon --since 1s` and parses the returned C-locale `short` journal timestamps. Any returned journal line older than the bounded tolerance fails the scenario.
8. `podlaz logs --daemon --since 1s --follow` must start with the lookback heading, remain running, and terminate on bounded SIGINT with status `130`; reaching the later KILL bound is a failure. This proves the follow path can be cancelled cleanly while using the same validated lookback implementation.
9. Representative compound, signed, zero, and excessive values must return CLI usage exit code `2`, produce no journal stdout, and include the product-level invalid-duration diagnostic. Unit/CLI regressions separately prove invalid input never reaches the backend and a valid duration followed by backend failure remains runtime exit code `1`.
10. `acceptance=pass` is written exactly once, from the EXIT cleanup path, only when the body and every scenario-owned cleanup step succeeded. The trap removes itself and exits explicitly with the original nonzero body status or promotes a successful body to exit `1` when cleanup failed. Unit shell regressions cover both status branches.
11. The workflow artifact contains only normalized pass/fail evidence. Profile URI/ID, base-doctor output, journal output, host addressing, resolver state, nftables inspection, and test-link inspection remain under the private E2E temp root and are removed before the existing pre-upload redaction scan.

A GitHub-hosted unit/packaging run does not replace this scenario. The pull request remains draft until the dedicated `TUN Package Convergence E2E` workflow passes for the PR head and its sanitized evidence is available.
...
The issue #247 diagnostics acceptance step runs next on the same installed package. It uses a separate isolated XDG root and keeps profile/command output private. While the committed TUN session is active it first proves clean doctor state, injects one empty E2E-only chain into the existing `inet podlaz` table, requires exact-composition doctor WARN, removes only the exact unchanged test chain, and reproves clean active status/doctor before disconnect. After clean inactive convergence it exercises an identity-guarded foreign-looking TUN. It then validates daemon/core `36h` windows, a measured short lookback, clean SIGINT follow cancellation, and invalid duration behavior. The scenario writes `acceptance=pass` only from successful cleanup and explicitly propagates cleanup failure to the shell exit status.
