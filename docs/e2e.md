...
| Installed-package teardown | `scripts/e2e/tun-package-cleanup.sh` | Bounded daemon recovery, exact metadata-driven network cleanup, package purge, sentinel removal, and tri-state post-cleanup assertions. |
...
The scenario trap and the workflow `if: always()` step both invoke `tun-package-cleanup.sh`. Before any route or policy-rule mutation, teardown parses every transaction file and atomically snapshots validated rollback metadata into a private temporary manifest. Unreadable JSON, an unsupported schema or owner, unexpected directory entries, or incomplete rollback metadata makes teardown fail closed. For every applied route and policy-rule step, the validator requires an exact matching desired-plan target and owner and an exact rollback tuple. It compares routes and rules as multisets, including count, so two applied entries cannot be represented by one rollback entry and a missing main-table bypass tuple cannot be hidden by a managed-table entry. The original transaction metadata is retained when validation or cleanup is incomplete.
...
After normal daemon recovery and exact metadata-driven fallback, teardown accumulates rather than suppresses failures. Final checks use a tri-state contract: confirmed absence is success, confirmed presence is failure, and an operational inspection error is also failure. The contract covers route/rule inspection, links, resolver state, nftables, generated and transaction paths, systemd service state, package database state, exact E2E sentinels, and direct DNS plus IPv4 egress. Transaction metadata is removed only after all recorded network tuples and other podlaz-owned fallback state are proven clean. A failed or timed-out package purge records `package_purged=false`; no inspector error can publish `cleanup_assertions=pass`.
