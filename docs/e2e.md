...
| Installed-package teardown | `scripts/e2e/tun-package-cleanup.sh` | Bounded daemon recovery, exact metadata-driven network cleanup, process-quiescence-gated package purge, identity-material preservation, sentinel removal, and tri-state post-cleanup assertions. |
...
The scenario trap and the workflow `if: always()` step both invoke `tun-package-cleanup.sh`. Before any route or policy-rule mutation, teardown parses every transaction file and atomically snapshots validated rollback metadata into a private temporary manifest. Unreadable JSON, an unsupported schema or owner, unexpected directory entries, transaction-directory inspection errors, command launch errors, or incomplete rollback metadata makes teardown fail closed. For every applied route and policy-rule step in a cleanup-required transaction, the validator requires an exact matching desired-plan target and owner and an exact rollback tuple. It compares routes and rules as multisets, including count, so two applied entries cannot be represented by one rollback entry and a missing main-table bypass tuple cannot be hidden by a managed-table entry.
...
Fallback must prove `podlazd` inactive and every transaction-owned Xray absent before it mutates networking or removes generated configuration, transaction metadata, or package executables. If daemon stop, Xray termination, `pgrep`, or `/proc` identity inspection fails, cleanup preserves all identity material and returns failure. If later network cleanup cannot be proven complete, generated configuration and transaction metadata are also preserved for a safe retry. Package purge is refused until process quiescence has been confirmed.
