...
| Installed-package TUN convergence | `scripts/e2e/tun-package-convergence.sh` | Release-like `.deb`, packaged inactive-scope verification, real missing-link rollback, private exact route/rule manifests, provenance, tri-state resource absence, unrelated host-state preservation, restart reconciliation, and immediate retry. |
| Installed-package teardown | `scripts/e2e/tun-package-cleanup.sh` | Pre-recovery verification-only ownership snapshot, post-quiescence authoritative mutation snapshot, exact metadata-driven cleanup, ownership-union verification, identity-material-preserving package purge gate, sentinel removal, and tri-state post-cleanup assertions. |
...
The scenario trap and the workflow `if: always()` step both invoke `tun-package-cleanup.sh`. Before hook removal or `recover --execute --yes`, teardown atomically snapshots every valid exact route/rule tuple into a private pre-recovery verification-only manifest. That manifest can never authorize deletion. If it cannot be created, recovery is skipped so it cannot destroy the remaining ownership proof. Fallback then removes the E2E hook, attempts bounded daemon recovery only when the early proof is valid, stops `podlazd`, stops every transaction-owned Xray, and proves both process classes absent. Only after all podlaz network mutators are quiescent does it parse the surviving transaction files and atomically create the post-quiescence authoritative manifest that alone may authorize fallback route/rule mutation.

After recovery and authoritative fallback mutation, teardown verifies absence of the union of exact tuples recorded by the pre-recovery and authoritative manifests. This catches the fail-closed case where recovery deletes a transaction file but leaves an arbitrary main-table bypass route that the empty post-quiescence manifest can no longer own. The early manifest remains verification-only: a tuple present only there causes teardown failure instead of authorizing deletion. Generated configuration, transaction state, and package executables are removed only after the entire ownership union is proven absent.

The validator treats `desired_plan` as intent and target-shape validation, never as proof that podlaz created an object. Network ownership is the exact multiset represented by durable route/policy-rule `applied_steps`; rollback routes/rules must equal that multiset exactly, including duplicate count. A `planned` transaction with no durable network ownership is non-mutating even when its desired plan contains routes or rules, while a planned record containing network applied/rollback ownership is rejected as inconsistent. An `applying` transaction remains a fail-closed crash window: if a desired route or policy-rule category lacks durable applied proof, teardown refuses to guess whether mutation occurred. In later states such as `applied`, `verifying`, or `committed`, zero-owned pre-existing exact routes/rules are valid and contribute no cleanup tuple; mixed transactions contribute only the objects represented by durable applied steps.
...
`scripts/e2e/tun-package-convergence.sh` is a separate release-like gate. It builds, installs, and reinstalls the branch `.deb`; verifies package and running-daemon provenance; runs packaged `Current Scopes: none`; snapshots exact route/rule ownership before disconnect; waits after real DNS apply; confirms a daemon-owned transaction exists; snapshots exact ownership before hook release; deletes real `podlaz0`; validates the real `resolvectl revert podlaz0` result; and requires the fixed event order `diagnostics-persisted`, `rollback-started`, `dns-rollback-started`. It performs strict exact-manifest verification immediately after each disconnect or rollback, then verifies complete tri-state resource absence, unrelated-state preservation, restart reconciliation, and immediate packaged reconnect/disconnect.
