...
| Installed-package teardown | `scripts/e2e/tun-package-cleanup.sh` | Bounded daemon recovery, post-quiescence authoritative network snapshot, exact metadata-driven cleanup, process-quiescence-gated package purge, identity-material preservation, sentinel removal, and tri-state post-cleanup assertions. |
...
The scenario trap and the workflow `if: always()` step both invoke `tun-package-cleanup.sh`. Normal daemon recovery may still mutate transaction and host state, so no pre-stop snapshot is authoritative. Fallback first removes the E2E hook, attempts bounded daemon recovery, stops `podlazd`, stops every transaction-owned Xray, and proves both process classes absent. Only after all podlaz network mutators are quiescent does it parse every transaction file and atomically replace the private exact route/rule manifest. Failure to prove quiescence or to create that post-quiescence snapshot preserves identity material and prevents any fallback network mutation.
