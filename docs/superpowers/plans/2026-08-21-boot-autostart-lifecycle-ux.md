...
- [ ] Write API tests proving valid proxy/TUN snapshots are accepted, missing mode/profile fields fail, no handoff field exists in encoded configuration requests, and disabled status cannot carry profile/mode data.
- [ ] Run focused API tests and confirm RED before implementation.
- [ ] Implement the minimal versioned daemon request/status types by reusing `ValidateConnectRequest` through a canonical reconstructed request with omitted handoff.
- [ ] Run focused API tests and confirm GREEN.
- [ ] Commit `test/feat: define boot autostart API`.
...
- [ ] Write deterministic tests for strict schema/trailing/unknown fields, `0600`, max-size bound, previous-boot attempt discard, manifest configured in current boot being ineligible, pinned admitted request, disabled-as-absence, and attempt state transitions `in_progress -> succeeded|terminal` only.
- [ ] Run focused store tests and confirm RED.
- [ ] Implement stores using `atomicWritePrivateFile`, `syncFilesystemDirectory`, cryptographically random non-secret manifest generation, strict `json.Decoder.DisallowUnknownFields`, exact boot-id validation, and fsynced unlink.
- [ ] Run focused store tests and confirm GREEN.
- [ ] Commit `feat: persist boot autostart authority`.
...
- [ ] Write handler/client tests for authorization deny/allow, read-only status without mutation authorization, disabled status, strict request validation, redaction/non-leakage, and HTTP method contracts.
- [ ] Run focused tests and confirm RED.
- [ ] Implement handlers/client and add conservative packaged polkit policy entry.
- [ ] Run focused tests and confirm GREEN.
- [ ] Commit `feat: expose boot autostart configuration`.
...
- [ ] Write tests for: disabled -> no attempt; current-boot configured manifest -> no attempt; previous-boot manifest -> one connect; crash after attempt admission before continuation -> replay exact pinned request; continuation exists -> no competing connect; succeeded/terminal attempt -> no same-boot reconnect; explicit disconnect after success -> no restart reconnect; manifest mutation during in-progress replay does not substitute profile; malformed current attempt fails closed; next boot permits a new logical attempt.
- [ ] Write serialization tests proving autostart uses the existing lifecycle operation lock/startup gate and cannot race recovery/shutdown.
- [ ] Run focused startup tests and confirm RED.
- [ ] Implement orchestration and wire it into `Server.Run` after `resumeNetworkSession` convergence but before accepting normal lifecycle requests; keep classification-only startup logs.
- [ ] Run focused tests and confirm GREEN.
- [ ] Commit `feat: run one logical autostart attempt per boot`.
...
- [ ] Write CLI tests for parsing, same validation path as `connect`, daemon-unavailable exit code, no immediate connect, concise output, deferred `--json`, help, and dynamic profile completion.
- [ ] Run focused CLI tests and confirm RED.
- [ ] Implement command routing/help/completion/client wiring.
- [ ] Run focused CLI tests and confirm GREEN.
- [ ] Commit `feat: add autostart CLI`.
...
**Interfaces:**
- Product states: `Connected`, `Connecting`, `Reconnecting`, `Disconnected`.
...
- [ ] Write tests mapping verified active -> Connected, active revalidation/degraded -> Reconnecting, admitted connect -> Connecting, inactive/terminal cleanup complete -> Disconnected; transient reconciliation must not be terminal.
- [ ] Write CLI output tests proving connect outputs only `Connected`, profile, mode; disconnect outputs only `Disconnected`; status does not print transaction/routes/DNS/firewall/runtime config while operator data remains in the report/API model.
- [ ] Run focused status/CLI tests and confirm RED.
- [ ] Implement centralized projection and concise renderers without deleting detailed daemon fields.
- [ ] Run focused tests and confirm GREEN.
- [ ] Commit `feat: simplify lifecycle human UX`.
...
- [ ] Add tests/contracts proving packaged `StateDirectory=podlaz` remains private, new polkit action ships, completions/man pages include autostart, and normal service restart semantics remain independent of autostart.
- [ ] Add installed acceptance covering boot boundary with autostart off/on, daemon restart while connected, package upgrade while connected, explicit disconnect, terminal autostart failure, and no retry loop.
- [ ] Update canonical docs/man pages to describe Boot Autostart Manifest, one logical attempt per boot, continuation priority, concise human UX, and operator-only recovery detail.
- [ ] Run deterministic shell/package contract tests available in hosted CI.
- [ ] Commit `docs/test: align boot autostart package contract`.

### Task 8: Verification and PR

**Files:** no new product files unless verification finds defects.

- [ ] Verify branch diff contains no private user IP/domain/profile/SSID values.
...
- [ ] Open a draft PR from `issue-263-boot-autostart-ux` to `master` with summary, safety/recovery implications, checks run, local environment limitation, and `Closes #263`.
