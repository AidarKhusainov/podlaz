...
python3 -m unittest scripts.e2e.tests.test_tun_soak_environment
...
Before running it, provision the private trusted-host fingerprint described in
`docs/e2e.md` from independently reviewed runner configuration. It must exist at
the canonical path as a root-owned mode-`0600` regular file and must not be
generated by the soak itself.

```bash
sudo install -d -o root -g root -m 0700 /etc/podlaz-e2e
sudo install -o root -g root -m 0600 \
  /secure/provisioning/tun-resource-soak-trusted-host.json \
  /etc/podlaz-e2e/tun-resource-soak-trusted-host.json
...
The self-hosted label is not OS proof: the harness independently checks
`/etc/os-release` for Ubuntu 24.04 before package or network operations. In final
`accept` mode the checked-in policy fixes the evidence configuration, is copied
to a private immutable snapshot, and is verified against the exact `HEAD` blob;
the public report contains only its SHA-256. Use runtime overrides only for
`observe`/debug runs.
