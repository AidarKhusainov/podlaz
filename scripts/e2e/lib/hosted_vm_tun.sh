...
HOSTED_VM_TUN_ENDPOINT_IP="10.0.2.100"
...
EOF
  ...
  HOSTED_VM_USER_NET_EXTRA=",guestfwd=tcp:${HOSTED_VM_TUN_ENDPOINT_IP}:${port}-tcp:127.0.0.1:${port}"
}
...
env XDG_CONFIG_HOME="$xdg/config" XDG_STATE_HOME="$xdg/state" XDG_CACHE_HOME="$xdg/cache"   /usr/bin/podlaz profile import "$uri" >"$private/import.stdout" 2>"$private/import.stderr"
...
env XDG_CONFIG_HOME="$xdg/config" XDG_STATE_HOME="$xdg/state" XDG_CACHE_HOME="$xdg/cache"   /usr/bin/podlaz profile validate "$profile" --mode tun >"$private/validate.stdout" 2>"$private/validate.stderr"
EOF
}

hosted_vm_tun_profile_id() {
  hosted_vm_ssh cat "${HOSTED_VM_TUN_PRIVATE}/profile-id" | tr -d '[:space:]'
}

hosted_vm_tun_capture_direct_baseline() {
  hosted_vm_ssh bash -s -- "${HOSTED_VM_TUN_PRIVATE}" <<'EOF'
set -Eeuo pipefail
private="$1"
...
hosted_vm_tun_assert_direct_blocked() {
  if hosted_vm_ssh bash -s -- "${HOSTED_VM_TUN_PRIVATE}" <<'EOF'
set -Eeuo pipefail
private="$1"
...
hosted_vm_tun_wait_status() {
  ...
  for _ in $(seq 1 "${attempts}"); do
    if hosted_vm_ssh sudo bash -c       "curl --fail --silent --show-error --max-time 5 --unix-socket /run/podlaz/podlazd.sock http://localhost/v1/status >${HOSTED_VM_TUN_PRIVATE}/status.json 2>/dev/null && python3 /tmp/daemon_status_semantics.py '${target}' ${HOSTED_VM_TUN_PRIVATE}/status.json"       >/dev/null 2>&1; then
      return 0
    fi
    sleep 1
  done
...
private="$2"
profile="$(cat "$private/profile-id")"
env XDG_CONFIG_HOME="$xdg/config" XDG_STATE_HOME="$xdg/state" XDG_CACHE_HOME="$xdg/cache"   /usr/bin/podlaz connect --mode tun "$profile" >"$private/connect.stdout" 2>"$private/connect.stderr"
EOF
...
hosted_vm_tun_capture_exact_active_authority() {
  local snapshot="$1"
  hosted_vm_ssh sudo bash -s -- "${HOSTED_VM_TUN_PRIVATE}" "${snapshot}" <<'EOF'
set -Eeuo pipefail
private="$1"
snapshot="$2"
...
hosted_vm_tun_assert_exact_terminal_cleanup() {
  hosted_vm_ssh sudo bash -s -- "${HOSTED_VM_TUN_PRIVATE}" <<'EOF'
set -Eeuo pipefail
private="$1"
...
xdg="$1"
private="$2"
env XDG_CONFIG_HOME="$xdg/config" XDG_STATE_HOME="$xdg/state" XDG_CACHE_HOME="$xdg/cache"   /usr/bin/podlaz recover --json >"$private/recover.json" 2>"$private/recover.stderr"
