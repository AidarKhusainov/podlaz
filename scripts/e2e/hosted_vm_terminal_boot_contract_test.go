...
func TestHostedVMTerminalBootPreservesOneTerminalAttempt(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(text, forbidden) {
			t.Fatalf("terminal boot scenario contains forbidden repair %q", forbidden)
		}
	}
}
...
func TestHostedVMTerminalBootWorkflowIsFocused(t *testing.T) {
	...
	for _, marker := range []string{
		"ubuntu-24.04",
		"qemu-system-x86",
		"hosted-vm-terminal-boot.sh",
		"terminal boot attempt stays consumed",
	} {
		if !strings.Contains(text, marker) {
			t.Fatalf("terminal boot workflow missing %q", marker)
		}
	}
	for _, forbidden := range []string{"suspend", "wifi", "package upgrade", "real-provider"} {
		if strings.Contains(text, forbidden) {
			t.Fatalf("terminal boot workflow mixes another domain %q", forbidden)
		}
	}
}
