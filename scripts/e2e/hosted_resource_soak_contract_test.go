...
func requireHostedResourceSoakMarkers(t *testing.T, text string, markers ...string) {
    ...
    for _, marker := range markers {
        if !strings.Contains(text, marker) { t.Fatalf("hosted resource-soak contract lost %q", marker) }
    }
}
...
func forbidHostedResourceSoakMarkers(t *testing.T, text string, markers ...string) {
    t.Helper()
    for _, marker := range markers {
        if strings.Contains(text, marker) { t.Fatalf("hosted resource-soak contract contains forbidden %q", marker) }
    }
}
...
func TestHostedResourceSoakWorkflowIsLongRunningDisposableQualification(t *testing.T) {
    ...
    for _, action := range []string{"actions/checkout", "actions/setup-go", "actions/upload-artifact"} {
        ...
        if !re.MatchString(workflow) { t.Fatalf("%s must be pinned to an immutable commit", action) }
    }
...
func TestHostedResourceSoakScenarioReusesExistingMeasurementEngine(t *testing.T) {
    ...
    requireHostedResourceSoakMarkers(t, script,
        ...
        "isolation.collect_snapshot()", "isolation.validate_clean_baseline(snapshot)",
    ...
    if output, err := cmd.CombinedOutput(); err != nil { t.Fatalf("bash -n %s: %v\n%s", hostedResourceSoakScript, err, output) }
}
...
func TestCheckedInResourceSoakPolicyRemainsReviewedContract(t *testing.T) {
    ...
    if err := json.Unmarshal(data, &policy); err != nil { t.Fatal(err) }
    if policy["schema_version"] != float64(2) || policy["mode"] != "observe" { t.Fatalf("unexpected soak policy identity: %#v", policy) }
    metricLimits, ok := policy["metric_limits"].(map[string]any)
    if !ok || len(metricLimits) != 0 { t.Fatal("reviewed observe policy unexpectedly gained active-trend limits") }
    lifecycle, ok := policy["lifecycle_limits"].(map[string]any)
    if !ok { t.Fatal("checked-in soak policy lost lifecycle thresholds") }
    for _, phase := range []string{"cleanup", "reconnect"} {
        rules, ok := lifecycle[phase].(map[string]any)
        if !ok || len(rules) == 0 { t.Fatalf("checked-in soak policy lost %s thresholds", phase) }
    }
}
