...
// These tests guard Q27 scenario shape; permanent evidence comes from the VM runtime workflow.
...
func TestHostedVMBootContinuationOrderingUsesPersistentForeignFixture(t *testing.T) {
	...
	for _, marker := range []string{
		...
		if !strings.Contains(text, marker) {
			t.Fatalf("shared boot-persistent foreign-state fixture missing %q", marker)
		}
	}
	...
	for _, marker := range []string{
		...
		if !strings.Contains(scenario, marker) {
			t.Fatalf("boot continuation ordering scenario missing foreign-state evidence %q", marker)
		}
	}
}
