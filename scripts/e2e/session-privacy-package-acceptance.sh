...
FOREIGN_NFT_TABLE="podlaz_issue260_foreign"
FOREIGN_DNS_LINK="podlaz-e2e-260dns0"
FOREIGN_DNS_SERVER="192.0.2.53"
FOREIGN_DNS_DOMAIN="~issue260.invalid"
EVIDENCE="${E2E_ARTIFACT_DIR}/issue260-acceptance.txt"
PROFILE_ID=""
CONNECTED=false
...
write_evidence() {
  local key="$1"
  case "${key}" in
    *[!A-Za-z0-9_.-]*) fail "invalid issue 260 evidence key" ;;
  esac
  printf '%s=pass\n' "${key}" >>"${EVIDENCE}"
}
...
wait_for_daemon_socket() {
  ...
  for attempt in $(seq 1 150); do
    [[ -S "${DAEMON_SOCKET}" ]] && sudo -n systemctl is-active --quiet podlazd.service && return 0
    sleep 0.1
  done
  fail "issue 260: podlazd.service did not become ready"
}
...
assert_fixture_absent() {
  ! sudo -n ip link show dev "${FOREIGN_TUN}" >/dev/null 2>&1 || fail "issue 260 foreign TUN fixture already exists"
  ! sudo -n ip link show dev "${FOREIGN_DNS_LINK}" >/dev/null 2>&1 || fail "issue 260 foreign DNS fixture already exists"
  ! sudo -n nft list table "${FOREIGN_NFT_FAMILY}" "${FOREIGN_NFT_TABLE}" >/dev/null 2>&1 || fail "issue 260 foreign nft fixture already exists"
  ! sudo -n ip -4 rule show priority "${FOREIGN_RULE_PRIORITY_A}" | grep -q . || fail "issue 260 priority ${FOREIGN_RULE_PRIORITY_A} is already occupied"
  ! sudo -n ip -4 rule show priority "${FOREIGN_RULE_PRIORITY_B}" | grep -q . || fail "issue 260 priority ${FOREIGN_RULE_PRIORITY_B} is already occupied"
  ! sudo -n ip -4 route show table "${FOREIGN_TABLE}" | grep -q . || fail "issue 260 table ${FOREIGN_TABLE} is already occupied"
}
...
assert_foreign_fixture() {
  ...
  resolved="$(mktemp "${E2E_TMP_ROOT}/issue260-resolved.XXXXXX")"
...
import_profile() {
  ...
  out="$(mktemp "${E2E_TMP_ROOT}/issue260-import.stdout.XXXXXX")"
  err="$(mktemp "${E2E_TMP_ROOT}/issue260-import.stderr.XXXXXX")"
  run_installed_podlaz profile import "${uri}" >"${out}" 2>"${err}" || fail "issue 260 profile import failed"
  PROFILE_ID="$(awk '/^Imported profile:/ {print $3}' "${out}")"
  [[ -n "${PROFILE_ID}" ]] || fail "issue 260 profile import returned no profile ID"
...
assert_active_data_plane() {
  ...
  status="$(mktemp "${E2E_TMP_ROOT}/issue260-status.XXXXXX")"
  run_installed_podlaz status --json >"${status}" 2>/dev/null || fail "issue 260 status failed while connected"
  python3 - "${status}" <<'PY'
import json
import sys
...
PY
  rm -f -- "${status}"
  sudo -n resolvectl --cache=no --interface=podlaz0 -4 query "${PODLAZ_E2E_DNS_CHECK_HOST}" >/dev/null 2>&1 || fail "issue 260 protected DNS path failed"
  public_ip="$(curl -4 -fsS --max-time 30 "${PODLAZ_E2E_PUBLIC_IP_CHECK_URL}")"
  [[ -n "${public_ip}" ]] || fail "issue 260 protected IPv4 data plane returned an empty response"
  mask_multiline_sensitive "${public_ip}"
  write_evidence protected_data_plane
}

setup_isolated_xdg "issue260-package-acceptance"
: >"${EVIDENCE}"
...
run_installed_podlaz connect --mode tun "${PROFILE_ID}" >/dev/null 2>&1 || fail "issue 260 coexistence connect failed"
CONNECTED=true
assert_dynamic_transaction_allocation
assert_active_data_plane
assert_foreign_fixture while_connected

run_installed_podlaz disconnect >/dev/null 2>&1 || fail "issue 260 coexistence disconnect failed"
CONNECTED=false
...
log "issue 260 installed-package coexistence acceptance passed"
