...
class RebootCoordinator:
    ...
    def _resume_autostart_off(self, checkpoint: Checkpoint, current_boot: str) -> None:
        ...
        checkpoint.scenarios["reboot_autostart_off"] = ScenarioRecord("reboot_autostart_off", ScenarioOutcome.PASS)
    ...
    def _resume_autostart_on(self, checkpoint: Checkpoint, current_boot: str) -> None:
        ...
        if attempt.get("schema_version") != "podlaz.boot-autostart-attempt.v1" or attempt.get("state") != "succeeded":
            raise AmbiguousState("successful boot did not persist succeeded autostart authority")
        fingerprint = self._attempt_fingerprint(attempt)
        self.product.disconnect()
        self._require_inactive()
        self.runner.run(("systemctl", "restart", "podlazd.service"), timeout=90).require_success("same-boot no-retry daemon restart")
        self._require_inactive()
        if self._attempt_fingerprint(RuntimeState.boot_attempt()) != fingerprint:
            raise AmbiguousState("explicit disconnect changed consumed boot attempt authority")
        checkpoint.scenarios["reboot_autostart_on"] = ScenarioRecord("reboot_autostart_on", ScenarioOutcome.PASS)
        checkpoint.scenarios["explicit_disconnect_no_same_boot_retry"] = ScenarioRecord("explicit_disconnect_no_same_boot_retry", ScenarioOutcome.PASS)
    ...
    def _resume_terminal(self, checkpoint: Checkpoint, current_boot: str) -> None:
        self._require_inactive()
        attempt = RuntimeState.boot_attempt()
        if attempt.get("schema_version") != "podlaz.boot-autostart-attempt.v1" or attempt.get("state") != "terminal":
            raise AmbiguousState("terminal boot did not persist terminal autostart authority")
        fingerprint = self._attempt_fingerprint(attempt)
        self.runner.run(("systemctl", "restart", "podlazd.service"), timeout=90).require_success("terminal no-retry daemon restart")
        self._require_inactive()
        if self._attempt_fingerprint(RuntimeState.boot_attempt()) != fingerprint:
            raise AmbiguousState("terminal boot attempt changed after same-boot daemon restart")
        checkpoint.scenarios["reboot_terminal_autostart"] = ScenarioRecord("reboot_terminal_autostart", ScenarioOutcome.PASS)
        checkpoint.scenarios["terminal_no_same_boot_retry"] = ScenarioRecord("terminal_no_same_boot_retry", ScenarioOutcome.PASS)
    ...
    def restore_original_policy(self) -> None:
        checkpoint = self.store.load()
        terminal = checkpoint.private.get("terminal_profile")
        self.product.autostart_disable()
        if terminal:
    ...
    def _verify_candidate(self, checkpoint: Checkpoint) -> None:
        ...
        result = self.runner.run(("dpkg-query", "-W", "-f=${Version}", "podlaz"), timeout=5).require_success("resume installed candidate")
    ...
    def _wait_status(self, connection: str, tun: str, *, require_verified: bool) -> None:
        import time
        ...
        while time.monotonic() < deadline:
            ...
            if result.returncode == 0:
                ...
                if payload.get("connection") == connection and payload.get("tun") == tun and (not require_verified or health.get("state") == "verified"):
                    return
            time.sleep(0.2)
        raise AmbiguousState(f"boot phase did not converge to {connection}/{tun}")
