...
func terminalCleanupStatusReport() Report {
	return Report{
		Connection: "active",
		Mode:       "tun",
		TUN:        "enabled (podlaz0)",
...
func TestCleanupRequiredTerminalSessionIsUnknownNotReconnecting(t *testing.T) {
	report := WithTunHealth(terminalCleanupStatusReport(), &api.TunHealthStatus{
...
func TestTerminalIntentIsUnknownWhenHealthObservationIsUnavailable(t *testing.T) {
	report := WithTunHealth(terminalCleanupStatusReport(), nil)
	if view := report.ProductView(nil); view.State != ProductUnknown {
		t.Fatalf("terminal cleanup without health state=%q, want %q; report=%#v", view.State, ProductUnknown, report)
	}
	if report.Connection != "unknown (terminal cleanup incomplete)" || report.ProductReconnecting {
		t.Fatalf("terminal cleanup depended on unavailable health observation: %#v", report)
	}
}

func TestTerminalIntentWinsOverStaleRevalidatingHealthPublication(t *testing.T) {
	report := WithTunHealth(terminalCleanupStatusReport(), &api.TunHealthStatus{
	...
	if view := report.ProductView(nil); view.State != ProductUnknown {
		t.Fatalf("durable terminal intent state=%q, want %q; report=%#v", view.State, ProductUnknown, report)
	}
	if report.ProductReconnecting {
		t.Fatalf("durable terminal intent was masked as reconnecting: %#v", report)
	}
}
