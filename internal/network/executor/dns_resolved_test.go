...
func TestDNSAwareTunExecutorAppliesVerifiesAndRollsBackDNSInSafeOrder(t *testing.T) {
	...
	if len(steps) != 5 {
		t.Fatalf("expected route, policy-rule, and DNS steps without TUN ownership, got %#v", steps)
	}
	for _, step := range steps {
		if step.Kind == "tun-device" {
			t.Fatalf("Xray-owned TUN link must not produce an ownership step: %#v", steps)
		}
	}
	if err := exec.Verify(context.Background(), plan); err != nil {
...
func TestDNSAwareTunExecutorPreservesPartialDNSOwnershipWithoutInternalRollback(t *testing.T) {
	...
	if len(steps) != 5 || steps[len(steps)-1].Kind != "dns" || steps[len(steps)-1].Owner != OwnerDNS {
