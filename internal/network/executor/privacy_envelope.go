...
func (e PrivacyEnvelopeExecutor) Verify(ctx context.Context, plan PrivacyEnvelopePlan) error {
	...
	// JSON is the nftables machine-readable contract. Human `nft list` output
	// may elide implicit protocol dependencies or rewrite symbolic expressions,
	// so it must not be cleanup authority for a fail-closed Privacy Envelope.
	result, err := observeCommand(ctx, e.Runner, "nft", "-j", "-y", "list", "table", plan.Family, plan.Table)
	if err != nil {
		return fmt.Errorf("verify privacy envelope %s %s: %w", plan.Family, plan.Table, err)
	}
	observed, err := parseOwnedNftJSONTable(result.Stdout, plan.Family, plan.Table)
	if err != nil {
		return fmt.Errorf("verify privacy envelope %s %s: %w", plan.Family, plan.Table, err)
	}
	firewallPlan := planner.TunFirewallPlan{Chains: plan.Chains, Rules: plan.Rules}
	if err := verifyExactNftJSONChains(observed, firewallPlan); err != nil {
		return fmt.Errorf("verify privacy envelope %s %s: %w", plan.Family, plan.Table, err)
	}
	return nil
}
