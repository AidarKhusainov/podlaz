...
func TestGenericRecoveryKeepsEveryActivePublicationMutationFree(t *testing.T) {
	...
	if !activeStatusMustKeepRecoveryMutationFree(status) {
		t.Fatal("generic recovery must not gain mutation authority from cleanup evidence while lifecycle is active")
	}
