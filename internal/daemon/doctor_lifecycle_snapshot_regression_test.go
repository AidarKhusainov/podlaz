...
func TestDoctorPublicationGuardRejectsMutationBetweenStages(t *testing.T) {
	...
	manager.mu.Unlock()
	snapshot := manager.captureDoctorLifecycleSnapshot()
	...
	if doctorPublicationLifecycleStable(before, after, snapshot, status) {
		t.Fatal("doctor publication treated a lifecycle mutation between stages as stable")
	}

	response := withIncompleteDoctorLifecycle(api.DoctorResponse{}, recovery.PlanResult{})
