...
func TestDNSWireRejectsMismatchedMessageIDAndCompressionLoop(t *testing.T) {
	...
	if _, err := ParseDNSResponse(message, 8, "example.com", DNSRecordTypeA); err == nil {
	...
	if _, err := ParseDNSResponse(loop, 9, "example.com", DNSRecordTypeA); err == nil {
		t.Fatal("expected compression loop to fail")
	}
}
